...
const ALLOWED_PATHS = new Set(['/agentic-wallet/balance', '/agentic-wallet/preflight', '/agentic-wallet/x402/fetch']);
...
class EltexX402Executor {
  ...
  async _request(pathWithQuery, { method = 'GET', body, idempotencyKey } = {}) {
    const pathname = String(pathWithQuery).split('?')[0];
    if (!ALLOWED_PATHS.has(pathname)) throw new Error('ELTEX_OPERATION_NOT_ALLOWED');
    ...
    try {
      ...
      const response = await this.fetchImpl(`${ELTEX_BASE}${pathWithQuery}`, {
        method,
        headers,
